...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter = new ArrayAdapter<String>(this, R.layout.note_item, R.id.notePreview, notes) {
            @Override
            public View getView(int position, View convertView, android.view.ViewGroup parent) {
                View view = super.getView(position, convertView, parent);
                android.widget.TextView textView = view.findViewById(R.id.notePreview);

                String fullNote = getItem(position);
                if (fullNote != null && fullNote.length() > 50) {
                    textView.setText(fullNote.substring(0, 47) + "...");
                } else {
                    textView.setText(fullNote);
                }
